...
function deteminant(aa, ab, ac, ba, bb, bc, ca, cb, cc: double): single;
begin
...
function is_valid_self_map(var data: I3Array): boolean;
begin
  result := true;
  for var i := 0 to length(data) - 1 do
  begin
    var elem := data[i];
    if (((elem.x = elem.y) and is_index(elem.x)) or
        ((elem.y = elem.z) and is_index(elem.y)) or
        ((elem.z = elem.x) and is_index(elem.z))) then
    begin
      result := false;
      break;
    end;
    ...
    if is_index(elem.x) and not TopoSet.part_of(data[elem.x], i) then
    begin
      result := false;
      break;
    end;

    if is_index(elem.y) and not TopoSet.part_of(data[elem.y], i) then
    begin
      result := false;
      break;
    end;

    if is_index(elem.z) and not TopoSet.part_of(data[elem.z], i) then
    begin
      result := false;
      break;
    end;
...
function is_valid_triangles(var data: I3Array): boolean;
begin
  result := true;
  for var i := 0 to length(data) - 1 do
  begin
...
begin
  ...
  begin
    for var i in poly_verts do
    begin
      assert(i <> tris_verts[current_tri].sample(side_i));
    end;
...
begin
  ...
  var points := points_set + [TPointF.Create(100000.0, 100000.0)] + [TPointF.Create(-100000.0, 100000.0)] + [TPointF.Create(0.0, -100000.0)];
  ...
  begin
    assert(is_point_in_triangle(points, super_tri, points[I]));
    tri_points[0][i] := i;
  end;
  ...
  begin
    // debugmsg('Loop');
    assert(length(tris_verts) = length(tris_connections));
    assert(length(tris_verts) = length(tri_points));
    // debugmsg('Step');

    assert(is_valid_self_map(tris_connections), 'invalid topology-connection map');
    assert(is_valid_triangles(tris_verts), 'invalid topology-verts map');
    ...
    for var tri_point in tri_points[current_tri] do
    begin
      assert(is_point_in_triangle(points, tris_verts[current_tri], points[tri_point]));
    end;
    ...
    begin
      ...
      // debugmsg('Written: ' + inttostr(poly_other_tris[index]) + ', ' + inttostr(tris_soup[next_index]) + ', ' + inttostr(tris_soup[prev_index]));
    end;
